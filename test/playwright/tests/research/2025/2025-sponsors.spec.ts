...
test.describe('2025 Sponsors Page', () => {
  ...
  for (const sponsorLevel of mainSponsors) {
    test(`Sponsor Level: ${sponsorLevel.level} is visible`, async ({ page }) => {
      await expect(page.getByRole('heading', { name: sponsorLevel.level, level: 3, exact: true })).toBeVisible();
    });
    ...
    for (const sponsor of sponsorLevel.sponsors) {
      test(`Sponsor: ${sponsor.name} is visible`, async ({ page }) => {
        ...
        if (sponsor.description) {
          await expect(page.locator('p', { hasText: sponsor.description })).toBeVisible();
        }
      });
    }
  }
