...
def post_github_review(repo: str, pr_number: str, github_token: str, review: ReviewResponse, diff_text: str):
    ...
    body_text = review.summary
    if not api_comments:
        if not body_text or body_text.strip() == "":
            body_text = (
                "### Pull Request Review Summary\n\n"
                "The changes in this pull request were reviewed using Gemini. "
                "The modifications appear logically consistent, follow best practices, and are well-structured.\n\n"
                "**No suggested changes**: No specific issues or improvements were found on the changed lines of code."
            )
        elif "no suggested changes" not in body_text.lower() and "no changes are suggested" not in body_text.lower() and "no suggestions" not in body_text.lower():
            body_text += "\n\n**No suggested changes**: The pull request was reviewed, and no specific issues or improvements were found on the changed lines of code."

    if api_comments:
        logging.info(f"Submitting {len(api_comments)} (out of {len(review.suggestions)}) inline comments and summary to PR #{pr_number}...")
        payload = {
            "body": body_text,
            "event": "COMMENT",
        ...
        logging.info(f"No specific inline suggestions. Submitting summary review as a standard PR comment to PR #{pr_number}...")
    ...
    if not response.ok:
        logging.error(f"Failed to post GitHub comment/review: {response.status_code} - {response.text}")
        response.raise_for_status()
        
    logging.info("GitHub review/comment submitted successfully!")
...
async def run_review():
    ...
    system_instructions = (
        ...
        "- If you find no issues or have no suggestions for improvements, use the 'summary' field to describe the changes in the pull request and explicitly state that there are no suggested changes."
    )
