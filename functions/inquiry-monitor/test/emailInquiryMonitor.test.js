...
describe('EMAIL MONITOR', () => {
  ...
  it('Is email-inquire expireAt 63 days from context', async () => {
    ...
    const expectedExpireTime = Timestamp.fromDate(expireDate);
    ...
    // Get get the updated document reference from EMAIL_INQUIRY_COLLECTION
    const updateInquirySnap = (await webInquiryCollectionRef.get()).data();
  ...
  it('Function writes data in expected schema', async () => {
    ...
    // Call the wrapped function with the snapshot (returns DocID)
    const newDocID = await wrapped(mockInquirySnap, context);

    // Get get the new document reference from the EMAIL_LOG_COLLECTION entry
    const newDocRef= db.collection(EMAIL_LOG_COLLECTION).doc(newDocID);

    // get new get reference to collection - then get document data
    const dataAfterCreate = (await newDocRef.get()).data();
