...
function computeProfileAndPercentile(urlParams) {
  ...
  for (let indicator of indicators) {
    ...
    indicator_value = urlParams.get(indicator);
...
function setProfileAndPercentile(){

  const urlParams = new URLSearchParams(window.location.search);
  let your_performance = computeProfileAndPercentile(urlParams);
  document.getElementById('performance-profile').innerText = your_performance.profile;
  document.getElementById('performance-percentile').innerText = your_performance.percentile;

}

// invoke calculations when page is ready
window.addEventListener('DOMContentLoaded',function(event){
  setProfileAndPercentile();
});
